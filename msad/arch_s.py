...
class SFCOS(nn.Module):
    def __init__(self, cfg):
        ...
        self.in_channel = 256
        self.ln_projector = nn.LayerNorm(self.in_channel, elementwise_affine=False)
        self.loss_projector = nn.SmoothL1Loss(beta=2.0)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            for fr_feature, st_feature in zip(fr_features, st_features):
                projected_fr_features.append(self.projector(fr_feature))
                projected_st_features.append(self.projector(st_feature))

            # Compute the loss for the projector
            projector_losses = [
                self.loss_projector(projected_fr, projected_st)
                for projected_fr, projected_st in zip(projected_fr_features, projected_st_features)
            ]
    ...
    def projector(self, x):
        ...
        x_flat = x.view(batch_size, -1, channels)

        # Apply LayerNorm
        x_norm = self.ln_projector(x_flat)
